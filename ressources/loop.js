!function(t, i) {
    "object" == typeof exports && "undefined" != typeof module ? module.exports = i() : "function" == typeof define && define.amd ? define(i) : (t = t || self).Flatbush = i()
}(this, (function() {
    "use strict";
    var t = function() {
        this.ids = [],
    ...
    t.prototype.pop = function() {
        if (0 !== this.length) {
            ...
            if (this.length--,
            this.length > 0) {
                for (var i = this.ids[0] = this.ids[this.length], s = this.values[0] = this.values[this.length], e = this.length >> 1, h = 0; h < e; ) {
                    var r = 1 + (h << 1)
                      , n = r + 1
                      , o = this.ids[r]
                      , a = this.values[r]
                      , u = this.values[n];
                    if (n < this.length && u < a && (r = n,
                    o = this.ids[n],
                    a = u),
                    a >= s)
                        break;
    ...
    ;
    var i = [Int8Array, Uint8Array, Uint8ClampedArray, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array]
      , s = function(s, e, h, r) {
        if (void 0 === e && (e = 16),
        void 0 === h && (h = Float64Array),
        void 0 === s)
        ...
        do {
            o += n = Math.ceil(n / this.nodeSize),
            this._levelBounds.push(4 * o)
        } while (1 !== n);this.ArrayType = h || Float64Array,
        this.IndexArrayType = o < 16384 ? Uint16Array : Uint32Array;
        var a = i.indexOf(this.ArrayType)
          , u = 4 * o * this.ArrayType.BYTES_PER_ELEMENT;
        if (a < 0)
            throw new Error("Unexpected typed array class: " + h + ".");
        ...
        this.maxY = this._boxes[this._pos - 1]) : (this.data = new ArrayBuffer(8 + u + o * this.IndexArrayType.BYTES_PER_ELEMENT),
        this._boxes = new this.ArrayType(this.data,8,4 * o),
        this._indices = new this.IndexArrayType(this.data,8 + u,o),
        this._pos = 0,
        this.minX = 1 / 0,
    ...
    function n(t, i) {
        ...
        a ^= s & (h = a) >> 4 ^ e & (r = u) >> 4,
        u ^= e & h >> 4 ^ (s ^ e) & r >> 4,
        a ^= (s = n = s & s >> 4 ^ e & e >> 4) & (h = a) >> 8 ^ (e = o) & (r = u) >> 8;
        var d = t ^ i
          , p = (e = (u ^= e & h >> 8 ^ (s ^ e) & r >> 8) ^ u >> 1) | 65535 ^ (d | (s = a ^ a >> 1));
        return ((p = 1431655765 & ((p = 858993459 & ((p = 252645135 & ((p = 16711935 & (p | p << 8)) | p << 4)) | p << 2)) | p << 1)) << 1 | (d = 1431655765 & ((d = 858993459 & ((d = 252645135 & ((d = 16711935 & (d | d << 8)) | d << 4)) | d << 2)) | d << 1))) >>> 0
    }
    return s.from = function(t) {
        ...
        if (r >> 4 != 3)
            throw new Error("Got v" + (r >> 4) + " data when expected v3.");
    ...
    s.prototype.finish = function() {
        ...
        for (var t = this.maxX - this.minX, i = this.maxY - this.minY, s = new Uint32Array(this.numItems), e = 0; e < this.numItems; e++) {
            var h = 4 * e
              , o = this._boxes[h++]
              , a = this._boxes[h++]
              , u = this._boxes[h++]
              , d = this._boxes[h++]
              , p = Math.floor(65535 * ((o + u) / 2 - this.minX) / t)
              , f = Math.floor(65535 * ((a + d) / 2 - this.minY) / i);
            s[e] = n(p, f)
        }
        !function t(i, s, e, h, n) {
            if (!(h >= n)) {
                for (var o = i[h + n >> 1], a = h - 1, u = n + 1; ; ) {
                    do {
                        a++
                    } while (i[a] < o);do {
                        u--
        ...
        }(s, this._boxes, this._indices, 0, this.numItems - 1);
        for (var _ = 0, l = 0; _ < this._levelBounds.length - 1; _++)
            for (var v = this._levelBounds[_]; l < v; ) {
                for (var x = 1 / 0, y = 1 / 0, m = -1 / 0, c = -1 / 0, b = l, w = 0; w < this.nodeSize && l < v; w++) {
                    var A = this._boxes[l++]
                      , g = this._boxes[l++]
                      , E = this._boxes[l++]
                      , I = this._boxes[l++];
                ...
                this._indices[this._pos >> 2] = b,
                this._boxes[this._pos++] = x,
                this._boxes[this._pos++] = y,
                this._boxes[this._pos++] = m,
                this._boxes[this._pos++] = c
            }
    }
    ,
    s.prototype.search = function(t, i, s, e, h) {
        if (this._pos !== this._boxes.length)
            throw new Error("Data not yet indexed - call index.finish().");
        for (var r = this._boxes.length - 4, n = this._levelBounds.length - 1, o = [], a = []; void 0 !== r; ) {
            for (var u = Math.min(r + 4 * this.nodeSize, this._levelBounds[n]), d = r; d < u; d += 4) {
                var p = 0 | this._indices[d >> 2];
                s < this._boxes[d] || e < this._boxes[d + 1] || t > this._boxes[d + 2] || i > this._boxes[d + 3] || (r < 4 * this.numItems ? (void 0 === h || h(p)) && a.push(p) : (o.push(p),
                o.push(n - 1)))
            }
            n = o.pop(),
            r = o.pop()
    ...
    s.prototype.neighbors = function(t, i, s, r, n) {
        if (void 0 === s && (s = 1 / 0),
        void 0 === r && (r = 1 / 0),
        this._pos !== this._boxes.length)
            throw new Error("Data not yet indexed - call index.finish().");
        for (var o = this._boxes.length - 4, a = this._queue, u = [], d = r * r; void 0 !== o; ) {
            for (var p = Math.min(o + 4 * this.nodeSize, h(o, this._levelBounds)), f = o; f < p; f += 4) {
                var _ = 0 | this._indices[f >> 2]
                  , l = e(t, this._boxes[f], this._boxes[f + 2])
                  , v = e(i, this._boxes[f + 1], this._boxes[f + 3])
                  , x = l * l + v * v;
                o < 4 * this.numItems ? (void 0 === n || n(_)) && a.push(-_ - 1, x) : a.push(_, x)
            }
...
function getNodesInside(elements, center) {
    ...
    for (var i = 0; i != elements.length; ++i) {
        if (elements[i].type == "node") {
            ...
            if (p.distanceTo(window.center) < 500) {
            ...
            if (elements[i].nodes.length > 1) {
                for (var n = 1; n != elements[i].nodes.length; ++n) {
                    ...
                    if (idNm1 in nodes && idN in nodes) {
                        ...
                        nodes[idNm1].neighbours.push({
                            node: idN,
                            edge: idE
                        });
                        nodes[idN].neighbours.push({
                            node: idNm1,
                            edge: idE
                        });
                    }
...
function filterBySelectedEdges(graph, edges) {
    ...
    for (var e = 0; e != edges.length; ++e) {
        newNodes[edges[e][0]].neighbours.push({
            node: edges[e][1],
            edge: e
        });
        newNodes[edges[e][1]].neighbours.push({
            node: edges[e][0],
            edge: e
        });
    }
...
function intersection(edge1, edge2, graph) {
    if ((edge1[0] != edge2[0]) && (edge1[1] != edge2[1]) && (edge1[1] != edge2[0]) && (edge1[0] != edge2[1])) {
...
function getGraphLoopFromEdge(line, graph) {
    ...
    do {
        ...
        pred = current;
        current = next;
    } while ((pred != line[0]) || (current != line[1]));return filterBySelectedEdges(graph, result);
}
...
function getContourEdge(graph) {
    ...
    for (var l = 0; l != edges.length; ++l) {
        if (nodes[edges[l][0]].lat <= lat && nodes[edges[l][1]].lat > lat) {
            crossings.push({
                edge: edges[l],
                coord: getLongitudeCrossing(lat, edges[l], nodes)
            });
        } else if (nodes[edges[l][1]].lat <= lat && nodes[edges[l][0]].lat > lat) {
            crossings.push({
                edge: [edges[l][1], edges[l][0]],
                coord: getLongitudeCrossing(lat, [edges[l][1], edges[l][0]], nodes)
            });
        }
    }
...
function setInformationError() {
    document.getElementById("msgLoop").innerHTML = "Error de cálculo ... lo siento :(";
...
function distToStr(dist) {
    if (dist < 500)
        ...
        return (dist / 500).toFixed(3).replace(".", ",") + " km";
    }
}
function setInformationDistance(size, sizePath) {
    var msg = "Longitud del bucle&nbsp;: <strong>" + distToStr(size) + "</strong>";
    if (sizePath > 0) {
        msg += ".<br /> Longitud del camino para llegar&nbsp;: <strong>" + distToStr(sizePath) + ".</strong>";
    }
...
function mergeSameLocation(graph) {
    ...
    for (var e = 0; e != graph.edges.length; ++e) {
        graph.nodes[graph.edges[e][0]].neighbours.push({
            node: graph.edges[e][1],
            edge: e
        });
        graph.nodes[graph.edges[e][1]].neighbours.push({
            node: graph.edges[e][0],
            edge: e
        });
    }
    return graph;
...
function addMiddleCrossing(graph) {
    ...
    for (const inter of intersections) {
        if (inter !== undefined) {
            ...
            if (!(inter.e1 in edgeSplit))
                edgeSplit[inter.e1] = [];
            if (!(inter.e2 in edgeSplit))
                edgeSplit[inter.e2] = [];
            edgeSplit[inter.e1].push({
                id: newID,
                coord: inter.coord.uA,
                origin: [e1, e2]
            });
            edgeSplit[inter.e2].push({
                id: newID,
                coord: inter.coord.uB,
                origin: [e1, e2]
            });
        }
    }
    for (var es in edgeSplit) {
        if (edgeSplit[es].length != 0) {
            ...
            if (edgeSplit[es].length > 1) {
                edgeSplit[es].sort(function(a, b) {
...
function distancePointEdge(C, A, B) {
    ...
    if (dist == 0)
        return {
            distance: C.distanceTo(A),
            point: A
        };
    const dist2 = dist * dist;
    const r = ((C.x - A.x) * (B.x - A.x) + (C.y - A.y) * (B.y - A.y)) / dist2;
    if (r < 0) {
        return {
            distance: C.distanceTo(A),
            point: A
...
function findShortestPath(graph, source, targets) {
    ...
    if (targets.indexOf(source) != -1)
        return path;
    var nodes = new PriorityQueue(), distances = {}, previous = {}, smallest, vertex, neighbor, alt;
    for (var n in graph.nodes) {
        if (n == source) {
            distances[n] = 0;
            nodes.enqueue(0, n);
        ...
        if (graph != null) {
            ...
            if (mainCC != null) {
                ...
                if (shortestPath.edges.length) {
                    gpxBody += "<rte><name>Acceso al bucle</name>";
                    ...
                    for (var e = 0; e != shortestPath.edges.length; ++e) {
                        ...
                        window.ClickCircularPaths.push(L.polyline(toPolyLine(shortestPath.edges[e], shortestPath.nodes), {
                            color: '#2ab50a'
                        }).addTo(map));
                    }
                    gpxBody += "</rte>\n";
                ...
                var neighbours = {};
                for (var e = 0; e != graph.edges.length; ++e) {
                    window.ClickCircularPaths.push(L.polyline(toPolyLine(graph.edges[e], graph.nodes), {
                        color: '#0060f0'
                    }).addTo(map));
                    if (!neighbours[graph.edges[e][0]])
                        neighbours[graph.edges[e][0]] = [];
                    neighbours[graph.edges[e][0]].push([graph.edges[e][1]]);
                    if (!neighbours[graph.edges[e][1]])
                        neighbours[graph.edges[e][1]] = [];
                ...
                gpxBody += "<rte><name>Bucle para caminar</name>";
                ...
                while (nextNode) {
                    ...
                    if (!neighboursVisited[currentNode])
                        neighboursVisited[currentNode] = [];
                    neighboursVisited[currentNode].push(nextNode);
                    if (!neighboursVisited[nextNode])
                        neighboursVisited[nextNode] = [];
                    neighboursVisited[nextNode].push(currentNode);
                    currentNode = nextNode;
                    nextNode = neighbours[currentNode].pop();
                    while (neighboursVisited[nextNode] && neighboursVisited[nextNode].includes(currentNode))
                        nextNode = neighbours[currentNode].pop();
                }
                gpxBody += "</rte>\n";
